...
(function (layui, window, factory) {
	if ((typeof exports === 'undefined' ? 'undefined' : _typeof(exports)) === 'object') {
		...
		//layui加载
		layui.link(layui.cache.modules.formSelects.replace(".js",".css"));
	...
	var v = '4.0.0.0713',
		...
		CONTENT_HEIGHT = 'xm-content-height',
	...
	Common.prototype.init = function (target) {
		...
		$(target ? target : 'select[' + NAME + ']').each(function (index, select) {
			var othis = $(select),
				...
				contentHeight = othis.attr(CONTENT_HEIGHT),
			...
			fs.config.contentHeight = contentHeight;
			...
			if (hisFs) {
				...
				contentHeight = fs.config.contentHeight;
	...
	Common.prototype.retop = function (div) {
		...
		if (up) {
			...
			dl.css({
				top: div[0].offsetTop + div.height() + base + 'px',
				bottom: 'auto',
				height: fs.config.contentHeight||''
			});
		}
	};
