...
*/
layui.define(['jquery', 'xlsx', 'fileSaver', "utils"], function (exports) {
    var $ = layui.jquery;
    exports('excel', {
        /**
         * 兼容老版本的导出函数
        ...
        downloadExl: function (data, filename, type) {
            type = type ? type : 'xlsx';
            this.exportExcel({sheet1: data}, filename, type);
        },
        /**
        ...
        exportExcel: function (data, filename, type, opt) {
            type = type ? type : 'xlsx';
            filename = filename ? (filename + '.' + type) : ('导出数据.' + type);
            ...
            for (i in data) {
                ...
                for (key in true_fields) {
                    ...
                    if (typeof old_field_name == 'function' && old_field_name.apply) {
                        exportData[i][new_field_name] = old_field_name.apply(window, [item[new_field_name] || item[utils.underline2Camel(new_field_name)], item, data]);
                    } else {
                        if (typeof item[old_field_name] != 'undefined' || typeof item[utils.underline2Camel(old_field_name)] != 'undefined') {
                            exportData[i][new_field_name] = item[old_field_name] || item[utils.underline2Camel(old_field_name)];
                        } else {
